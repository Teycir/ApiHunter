...
/// Rank a large list of targets by risk score (InternetDB + ipinfo.io probes).
///
/// Example:
///   apihunter triage --urls targets/large-list.txt --format json --top 50
#[derive(Debug, Parser)]
#[command(
    name = "triage",
    about = "Rank targets by risk score using lightweight passive probes",
    group(
        ArgGroup::new("triage_input")
            .required(true)
            .args(["urls", "stdin"])
    )
)]
pub struct TriageCli {
    // ── Input ────────────────────────────────────────────────────────────────
    /// Path to a newline-delimited file of IPs or domain targets.
    #[arg(short = 'u', long, value_name = "FILE", group = "triage_input")]
    pub urls: Option<PathBuf>,

    /// Read targets from stdin (one per line).
    #[arg(long, group = "triage_input")]
    pub stdin: bool,

    // ── Engine tuning ────────────────────────────────────────────────────────
    /// Parallel probe tasks.
    #[arg(short = 'c', long, default_value_t = 100, value_name = "N")]
    pub concurrency: usize,
    ...
    /// Only emit entries at or above this score (0–100).
    #[arg(long, default_value_t = 0, value_name = "SCORE")]
    pub min_score: u8,

    /// Emit only the top N entries by score (0 = all).
    #[arg(long, default_value_t = 0, value_name = "N")]
    pub top: usize,

    // ── Promote ──────────────────────────────────────────────────────────────
    /// Write promoted targets to this file for use as --urls input in a
    /// subsequent full scan. If --top > 0, only the top N are written.
    #[arg(long, value_name = "FILE")]
    pub promote_to: Option<PathBuf>,

    // ── Output ───────────────────────────────────────────────────────────────
    /// Output format.
    #[arg(short = 'f', long, default_value = "pretty", value_name = "FORMAT")]
    pub format: TriageFormat,

    /// Write triage output to this file (default: stdout).
    #[arg(short = 'o', long, value_name = "FILE")]
    pub output: Option<PathBuf>,
...
/// Load triage targets from a file or stdin (blank/comment lines stripped).
pub fn load_triage_targets(cli: &TriageCli) -> Result<Vec<String>> {
    let raw: Vec<String> = if let Some(ref path) = cli.urls {
        let content = fs::read_to_string(path)
            .with_context(|| format!("Cannot read triage target file: {}", path.display()))?;
        content.lines().map(str::to_owned).collect()
    } else {
        let stdin = io::stdin();
        stdin
