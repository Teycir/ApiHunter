...
//   - We use rdap.org as a bootstrap aggregator so we don't need to
//     enumerate per-TLD RDAP servers ourselves.

use crate::triage::types::{RdapDomainData, SourceOutcome};
...
pub async fn fetch(
    ...
    // 404 → domain not in RDAP; return empty (not an error)
    if resp.status() == 404 {
    ...
    // Extract registrar from the first entity with role "registrar"
    let registrar = raw
        .entities
        .iter()
    ...
    // Extract created / expires from events
    ...
    // Privacy-protected heuristic: registrant entity has a remark containing
    // "privacy" or "redact" (case-insensitive), or its vcard fn is a proxy service name.
...
/// Extract the FN (formatted name) property from a vCard 4.0 array.
/// vcardArray shape: ["vcard", [["fn", {}, "text", "Some Name"], ...]]
fn vcard_fn(vcard: Option<&serde_json::Value>) -> Option<String> {
    let arr = vcard?.as_array()?;
    // arr[1] is the property list
