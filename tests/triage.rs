// tests/triage.rs
//
// Unit and integration tests for the threat_intel module.
//
...
// Engine-level tests (NoTargets, entry count) hit run_probes directly.
//
// Naming: threat_intel_<scenario>_<expectation>

use api_scanner::threat_intel::{
    risk::{compute_risk_score, MODEL_MAX},
    run_probes,
    types::{
        InternetDbData, IpInfoData, SourceOutcome, ThreatIntelConfig, ThreatIntelEntry,
        ThreatSeverity,
    },
    ProbeError,
};
...
fn fast_config() -> ThreatIntelConfig {
    ThreatIntelConfig {
        concurrency: 10,
...
fn make_entry(target: &str, raw: u16) -> ThreatIntelEntry {
...
#[tokio::test]
async fn threat_intel_no_targets_returns_error() {
    let result = run_probes(vec![], fast_config()).await;
    assert!(
        matches!(result, Err(ProbeError::NoTargets)),
        "expected NoTargets, got: {result:?}"
    );
}

#[tokio::test]
async fn threat_intel_single_target_always_returns_one_entry() {
    ...
    let tr = run_probes(vec!["256.256.256.256".to_string()], fast_config())
        .await
...
#[tokio::test]
async fn threat_intel_model_max_matches_weight_sum() {
...
#[tokio::test]
async fn threat_intel_zero_signals_produce_zero_score() {
    let risk = compute_risk_score(None, None, None);
    assert_eq!(risk.raw_score, 0);
    assert_eq!(risk.score, 0);
    assert_eq!(risk.severity, ThreatSeverity::Low);
...
#[tokio::test]
async fn threat_intel_high_risk_ports_score_4pts_each() {
    let mut idb = empty_idb("1.2.3.4");
    idb.ports = vec![22, 3389, 6379];
...
#[tokio::test]
async fn threat_intel_low_risk_ports_score_1pt_each() {
    let mut idb = empty_idb("1.2.3.4");
    idb.ports = vec![80, 443, 8888];
...
#[tokio::test]
async fn threat_intel_mixed_ports_score_independently() {
    let mut idb = empty_idb("1.2.3.4");
    idb.ports = vec![22, 80];
...
#[tokio::test]
async fn threat_intel_two_unscored_cves_give_4_raw_pts() {
...
#[tokio::test]
async fn threat_intel_many_cves_accumulate_without_artificial_cap() {
    let mut idb = empty_idb("1.2.3.5");
...
#[tokio::test]
async fn threat_intel_honeypot_tag_scores_5pts_and_flags_vuln() {
...
#[tokio::test]
async fn threat_intel_scanner_tag_scores_3pts_and_flags_vuln() {
    let mut idb = empty_idb("1.2.3.6");
...
#[tokio::test]
async fn threat_intel_hosting_asn_scores_3pts() {
...
#[tokio::test]
async fn threat_intel_min_score_filter_drops_low_entries() {
    ...
    let result = run_probes(vec!["127.0.0.1".to_string()], config)
        .await
