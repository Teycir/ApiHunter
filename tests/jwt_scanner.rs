...
#[tokio::test]
async fn alg_confusion_not_reported_when_unauthenticated_access_is_already_successful() {
    let server = MockServer::start().await;
    let token = make_rs256_jwt_with_jwk();
    let call_count = Arc::new(AtomicUsize::new(0));
    let call_count_for_get = Arc::clone(&call_count);
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(move |_request: &wiremock::Request| {
            ...
            if call == 0 {
                ...
                // Endpoint is publicly accessible regardless of auth.
                ResponseTemplate::new(200).set_body_string("public")
            }
        })
        .mount(&server)
        .await;
    ...
    assert!(
        errors.is_empty(),
        "unexpected errors in public-access baseline case: {errors:#?}"
    );
    assert!(
